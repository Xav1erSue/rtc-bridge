...
VP8_CAPS = Gst.Caps.from_string(
    "application/x-rtp,media=video,encoding-name=VP8,payload=97,clock-rate=90000"
)
H264_CAPS = Gst.Caps.from_string(
    "application/x-rtp,media=video,encoding-name=H264,payload=98,clock-rate=90000"
)
OPUS_CAPS = Gst.Caps.from_string(
    "application/x-rtp,media=audio,encoding-name=OPUS,payload=100,clock-rate=48000"
)
...
class WebRTC(EventEmitter):
    ...
    def on_ice_candidate(self, element, mlineindex, candidate):
        self.emit("candidate", {"sdpMLineIndex": mlineindex, "candidate": candidate})

    def add_transceiver(self, direction, codec):
        upcodec = codec.upper()
        caps = None
        print("NOTICE: {}".format(upcodec))
        if upcodec == "H264":
            caps = H264_CAPS
        elif upcodec == "VP8":
            caps = VP8_CAPS
